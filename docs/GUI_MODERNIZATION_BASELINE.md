...
Current automated profile redraw result: `canvas.updateBackground`, `canvas.redraw`, and `canvas.redraw_keep_view` dominate. The original 20-second simulator recording smoke baseline showed `canvas.sample_processing` around 1ms average and 2ms max, while live `canvas.updategraphics` reached a 47ms max and full redraw/background paths remained larger. After the Phase 3 processed-frame contract slice, one comparable short simulator smoke measured `canvas.sample_processing` around 0.5ms average and 0.6ms max. A later 20-second smoke after PID/manual/extra diagnostics slices measured `canvas.sample_processing` around 1.0ms average and 1.8ms max. The latest 20-second smoke after curve data payload extraction measured `canvas.sample_processing` around 1.1ms average and 1.8ms max, while `updategraphics`, `updateBackground`, and `redraw` remained larger. This still supports continuing Phase 3 extraction toward a movable processing boundary, but the short offscreen simulator run is noisy; use a longer simulator/device baseline before treating the lower timing as causal or starting broad threading changes.
