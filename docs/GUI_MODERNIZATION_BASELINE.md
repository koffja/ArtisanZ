...
Phase 1.5 decision: PyQtGraph is now the default selected renderer. Matplotlib remains available through `ARTISANZ_RENDERER_ID=matplotlib-snapshot` and runtime fallback paths. The first default PyQtGraph profile-redraw capture now displays historical profile curves and preserves the LCD telemetry column; however, the PyQtGraph visual path still has lower overlay parity than Matplotlib for rich annotations/background analysis. Continue treating Matplotlib as the compatibility/export renderer while Phase 2 follow-up improves PyQtGraph snapshot coverage.
