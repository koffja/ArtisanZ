...
| 2026-07-03 | ArtisanZ | Phase 1.23 PyQtGraph cursor/i18n/theme parity pass | 90 WebSocket `getData` samples, fixed 1s virtual step, plus focused regression suite | Renderer/widget/canvas/theme/extractor unit coverage plus event-heavy PyQtGraph smoke for translated labels, toolbar cursor readout, hidden PyQtGraph auto-range control, dialog input width, rail background, BT/Delta ET pens, and default graph palette | `97 passed`; `py_compile` passed; `ruff check` passed; `git diff --check` passed; smoke `ror_item_count=2`; `temperature_item_count=7`; `renderer_event_item_count=9`; `screenshot_sampled_non_background_pixel_count=25257` | Screenshot: `/tmp/artisanz-phase123-pyqtgraph-cursor-theme.png`; restored toolbar cursor time/temp/RoR feedback, routed event labels through Qt i18n, removed the stray lower-left `A` auto-range button, widened clipped dialog value fields, matched the bottom event rail to the LCD surface, made BT visually stronger, and changed the default graph palette to a softer Morandi/Wu Guanzhong-style set while preserving the old default as `Themes/Artisan/Artisan.athm`. |
| 2026-07-03 | ArtisanZ | Phase 1.24 Cotrix toolbar and cursor fit pass | Saved profile screenshot smoke plus focused toolbar/theme/cursor tests | Main window profile redraw with Cotrix toolbar entry, hidden empty subscription action, removed separator artifact, and single-line cursor telemetry format | `29 passed`; `py_compile` passed; `ruff check` passed; `git diff --check` passed | Screenshot: `/tmp/artisanz-toolbar-redesign.png`; the toolbar now opens with a Cotrix brand entry instead of the legacy plus sign, no longer reserves an empty subscription gap, removes the square separator artifact, keeps graph navigation/actions compact, and changes PyQtGraph cursor readout to a single line to prevent bottom clipping. |
| 2026-07-03 | ArtisanZ | Phase 1.25 PyQtGraph-native toolbar policy | Saved profile screenshot smoke plus focused toolbar policy tests | Main window profile redraw with the default PyQtGraph renderer and a renderer-surface toolbar policy | `4 passed` focused policy tests; `29 passed` related toolbar/canvas/theme tests; `py_compile` passed; `ruff check` passed; `git diff --check` passed | Screenshot: `/tmp/artisanz-toolbar-policy.png`; default PyQtGraph mode now hides Matplotlib-only Back/Forward/Pan/Zoom controls, keeps Cotrix/Home/Lines/status readout, and routes Home to the visible PyQtGraph view reset while retaining the full Matplotlib toolbar on the compatibility renderer. |
| 2026-07-03 | ArtisanZ | Phase 1.26 toolbar visual fit pass | Saved profile screenshot smoke plus focused toolbar/theme tests | Main window profile redraw after explicit toolbar visual metrics and rebuilt Lines SVG assets | `18 passed`; `py_compile` passed; `ruff check` passed; `git diff --check` passed | Screenshot: `/tmp/artisanz-toolbar-polish.png`; Cotrix now uses a fixed-height brand pill with no bottom clipping, Home/Lines buttons share a square icon-button size, Lines uses a heavier centered 32x32 SVG, and the toolbar/readout heights are governed by one metric set instead of scattered magic numbers. |
...
Phase 1.23 decision: PyQtGraph must preserve user interaction and localization, not only static drawing. Main-event labels now pass through the existing Qt translation system, cursor movement on the PyQtGraph surface reports time, nearest ET/BT temperature, and nearest RoR to the toolbar again, and the PyQtGraph auto-range `A` button is hidden because it duplicates existing toolbar/navigation controls and reads as stray chrome in the main roast view. The default graph palette is now softer and lower-saturation; the prior Artisan default remains selectable as a named theme.

Phase 1.24 decision: the toolbar should not carry empty state placeholders or Artisan Plus-era brand semantics. The first action now reads as Cotrix, the inactive subscription slot is hidden instead of left as blank space, and the old separator is removed because it rendered as a block rather than a useful divider. Matplotlib navigation actions remain for compatibility until a PyQtGraph-native toolbar policy replaces them with controls that act directly on the visible PyQtGraph surface.

Phase 1.25 decision: toolbar controls must be tied to the active renderer surface. PyQtGraph mode now hides legacy Matplotlib history/pan/rectangle-zoom controls because they do not operate on the visible PyQtGraph widget; Home remains and resets the PyQtGraph view through the canvas sync path. Matplotlib mode keeps the original NavigationToolbar controls for compatibility. The Lines action remains as a known compatibility bridge until curve-style editing becomes renderer-neutral.

Phase 1.26 decision: toolbar geometry should be specified as product UI metrics rather than ad hoc per-button stylesheet values. The Cotrix brand pill, icon-only actions, and cursor readout now use one 58px toolbar band with 44px controls. Project toolbar icons must fill their SVG viewBox with sufficient stroke weight; the Lines icon was rebuilt because the prior asset read as undersized even when the button itself had enough space.
