...
Current automated profile redraw result: `canvas.updateBackground`, `canvas.redraw`, and `canvas.redraw_keep_view` dominate. The original 20-second simulator recording smoke baseline showed `canvas.sample_processing` around 1ms average and 2ms max, while live `canvas.updategraphics` reached a 47ms max and full redraw/background paths remained larger. After the Phase 3 processed-frame contract slice, one comparable short simulator smoke measured `canvas.sample_processing` around 0.5ms average and 0.6ms max. Later 20-second smokes measured `canvas.sample_processing` around 1.0ms average after PID/manual/extra diagnostics, around 1.1ms after curve data payload extraction, and around 0.6ms after input-filter extraction. `updategraphics`, `updateBackground`, and `redraw` remain larger. This still supports continuing Phase 3 extraction toward a movable processing boundary, but the short offscreen simulator run is noisy; use a longer simulator/device baseline before treating the lower timing as causal or starting broad threading changes.

Renderer benchmark decision: the current synthetic adapter benchmark shows PyQtGraph roughly 4x faster than the Matplotlib adapter for the same snapshot, with or without requesting OpenGL. The offscreen platform reports `opengl_widget_supported=false`, so OpenGL acceleration is not yet proven on this machine. The next renderer step should be a guarded PyQtGraph live-renderer POC or real-window/device OpenGL validation, not a Matplotlib removal.
