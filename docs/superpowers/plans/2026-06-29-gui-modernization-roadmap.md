...
| 6 | Plugin Boundary Exploration | Started: renderer plugin metadata seam | Renderer/report/filter/analyzer extension seams | Future customization can be isolated without destabilizing upstream sync |
...
**Started:** 2026-07-01 with a side-effect-light renderer plugin registry that describes the existing Matplotlib and PyQtGraph snapshot adapters as built-in renderer plugins. This creates the first renderer plugin metadata seam without changing `main.py`, `canvas.py`, or the live roast rendering path; the runtime swappable renderer boundary remains a follow-up step.
