...
| 3 | Sampling Post-Processing Decoupling | Started: smoothing-weight helper extracted | Pure data pipeline for filtering, RoR, PID inputs, alarm/event decisions | GUI thread consumes snapshots instead of doing heavy processing inline |
...
**Started:** 2026-06-29 with `sample_processing.py`, a tested pure helper for choosing decay smoothing weights. `canvas.py` still owns live mutation, PID calls, event/alarm signals, and rendering; this first slice only removes one RoR-sensitive smoothing decision from the 692-line GUI-thread method.
