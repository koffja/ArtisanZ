...
| 3 | Sampling Post-Processing Decoupling | Started: smoothing weights/averages, PID input, alarm trigger evaluation, auto-event, RoR computation/display/filter, connected points, input-filter backfill/previous values, axis extension, post-sample update decisions, and first processed-frame contract extracted | Pure data pipeline for filtering, RoR, PID inputs, alarm/event decisions | GUI thread consumes snapshots instead of doing heavy processing inline |
