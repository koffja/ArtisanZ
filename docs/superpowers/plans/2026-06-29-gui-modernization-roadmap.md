...
**Status:** Phase 0 automated profile redraw baseline and a 20-second internal simulator live-sampling smoke baseline are captured as of 2026-06-29. Phase 1 Widgets visual modernization has a main-screen first pass with screenshots. Phase 2 has a renderer snapshot contract and a canvas-like extractor foundation; longer simulator/device baselines remain pending before broad threading work.
...
| 2 | Plot Renderer Boundary and PyQtGraph POC | Snapshot contract + extractor started | Read-only plot snapshot model, Matplotlib adapter, PyQtGraph live adapter prototype | POC is faster or visually/maintainably superior under Phase 0 benchmarks |
...
**Started:** 2026-06-29 with a data-only `plot_snapshot.py` contract and `plot_snapshot_extractor.py` canvas-like snapshot builder. These do not change runtime plotting behavior yet; they prepare a stable renderer-facing surface before a Matplotlib compatibility adapter or PyQtGraph POC work.
