...
**Status:** Phase 0 automated profile redraw baseline and a 20-second internal simulator live-sampling smoke baseline are captured as of 2026-06-29. Phase 1 Widgets visual modernization has a main-screen first pass with corrected LCD spacing, label padding, and full-width bottom value surfaces. Phase 2 has a renderer snapshot contract, a canvas-like extractor with foreground event markers, an unhooked Matplotlib adapter that renders snapshot events, an offscreen Agg smoke path, and an optional-dependency PyQtGraph-style POC adapter; longer simulator/device baselines remain pending before broad threading work.
...
- `src/requirements.txt` pins `matplotlib==3.10.9`, `PyQt6==6.11.0`, and `PyQt6-WebEngine==6.11.0`.
...
| 2 | Plot Renderer Boundary and PyQtGraph POC | Renderer boundary + optional PyQtGraph POC adapter complete | Read-only plot snapshot model, Matplotlib adapter, PyQtGraph live adapter prototype | POC is ready for dependency-backed benchmark comparison before any live renderer swap |
...
**Started:** 2026-06-29 with a data-only `plot_snapshot.py` contract, `plot_snapshot_extractor.py` canvas-like snapshot builder with foreground event marker extraction, `plot_matplotlib_adapter.py` compatibility adapter with event-marker rendering, `plot_matplotlib_smoke.py` offscreen Agg diagnostic path, and `plot_pyqtgraph_adapter.py` as an optional-dependency PyQtGraph-style POC adapter. These do not change runtime plotting behavior yet; they prepare a stable renderer-facing surface before dependency-backed PyQtGraph benchmarking or live canvas integration.

**Completion Note:** The Phase 2 renderer-boundary POC is complete at code level. `pyqtgraph` is not currently installed or pinned in `requirements.txt`, so the adapter imports without that dependency and uses factories for real `InfiniteLine`/`TextItem` creation when the dependency is added. Do not replace the live `canvas.py` renderer until a benchmark branch installs PyQtGraph and compares it against the Phase 0 scenarios.
