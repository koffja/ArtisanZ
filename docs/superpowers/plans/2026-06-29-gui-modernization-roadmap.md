...
| 2 | Plot Renderer Boundary and PyQtGraph POC | Backend-aware live-frame dispatch wired for main curves; real PyQtGraph widget still pending | Read-only plot snapshot model, Matplotlib adapter, PyQtGraph live adapter prototype | POC is benchmarkable before any live renderer swap |
