...
| 2 | Plot Renderer Boundary and PyQtGraph POC | Renderer boundary + dependency-backed PyQtGraph smoke + benchmark/OpenGL capability entry complete | Read-only plot snapshot model, Matplotlib adapter, PyQtGraph live adapter prototype | POC is benchmarkable before any live renderer swap |
| 3 | Sampling Post-Processing Decoupling | Started: smoothing weights/averages, PID input/SV update, alarm trigger evaluation, auto-event, RoR computation/display/filter, connected points, input-filter previous/backfill/result decisions, axis extension, post-sample/external-output decisions, windowed/full curve data payloads, and first processed-frame contract extracted | Pure data pipeline for filtering, RoR, PID inputs, alarm/event decisions | GUI thread consumes snapshots instead of doing heavy processing inline |
| 4 | Main UI Information Architecture | Started: workspace model + policy-backed toolbar state | Scenario-oriented Roast Control, QC Analysis, and Device Setup workspaces | Users can switch task modes without losing existing expert controls |
...
**Policy Note:** A follow-up pure-model slice adds `WorkspacePolicy` and `workspace_policy(mode)` for future menu, toolbar, side-panel, analysis, device-setup, advanced-control, and compact-chrome decisions. The first integration slice stores `workspace_policy` on `ApplicationWindow`, synchronizes it with existing `UI_MODE` settings paths, and routes toolbar expert line configuration through `policy.show_advanced_controls` while preserving current Expert/Default/Production behavior.
