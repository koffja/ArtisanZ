...
| 2 | Plot Renderer Boundary and PyQtGraph POC | Canvas-level renderer selection seam complete with robust fallback; live drawing switch follow-up | Read-only plot snapshot model, Matplotlib adapter, PyQtGraph live adapter prototype | POC is benchmarkable before any live renderer swap |
