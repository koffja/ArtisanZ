...
| 5 | Local QML Islands | Started: workspace-status model + QML compile smoke | A limited Qt Quick panel or dashboard backed by Python state models | QML proves useful without breaking packaging, translations, or Windows portable build |
...
**Started:** 2026-07-01 with an isolated `WorkspaceStatusModel` `QObject` and a small QML status panel string compiled by an offscreen `QQmlComponent` test. This validates the Python-to-QML state seam without wiring QML into `ApplicationWindow` or adding package data files.
