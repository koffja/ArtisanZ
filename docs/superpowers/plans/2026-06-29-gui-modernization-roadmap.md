...
| 1.6 | PyQtGraph Visual Parity and Dialog Refresh | Started: grid overlay, phase-band visibility, LCD no-outer-frame strategy, and Morandi dialog base styling | Restore high-value Matplotlib visual effects in PyQtGraph and flatten key settings dialogs | PyQtGraph first screen no longer feels visually or functionally behind the legacy Matplotlib view |
...
**Started:** 2026-07-02 with a dedicated `GridItem` overlay so grid visibility is not dependent on faint PyQtGraph defaults, higher-contrast Morandi phase-band colors for default phase palettes, a transparent LCD container strategy that removes the fragile nested-card/value composition, and a flatter Morandi base stylesheet for `QDialog`, `QMessageBox`, `QFileDialog`, and dialog controls.
...
**Dialog Visual Backlog:**

- Apply the Morandi base first through property-safe global dialog selectors.
- Then modernize high-traffic settings dialogs individually: Axes, Curves, Events, Devices, Alarms, and Roasting Properties.
