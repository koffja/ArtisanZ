...
| 3 | Sampling Post-Processing Decoupling | Started: smoothing weights, PID input, alarm, auto-event, RoR display/filter extracted | Pure data pipeline for filtering, RoR, PID inputs, alarm/event decisions | GUI thread consumes snapshots instead of doing heavy processing inline |
