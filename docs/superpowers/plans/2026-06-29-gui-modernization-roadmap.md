...
| 4 | Main UI Information Architecture | Started: workspace model + chrome policy | Scenario-oriented Roast Control, QC Analysis, and Device Setup workspaces | Users can switch task modes without losing existing expert controls |
...
**Policy Note:** A follow-up pure-model slice adds `WorkspacePolicy` and `workspace_policy(mode)` for future menu, toolbar, side-panel, analysis, device-setup, advanced-control, and compact-chrome decisions. The policy is currently inert data; the next Phase 4 slice should wire it into `ApplicationWindow` incrementally without changing existing Expert/Default/Production behavior unexpectedly.
