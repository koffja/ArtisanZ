...
| 2 | Plot Renderer Boundary and PyQtGraph POC | Snapshot contract + Matplotlib adapter started | Read-only plot snapshot model, Matplotlib adapter, PyQtGraph live adapter prototype | POC is faster or visually/maintainably superior under Phase 0 benchmarks |
...
**Started:** 2026-06-29 with a data-only `plot_snapshot.py` contract, `plot_snapshot_extractor.py` canvas-like snapshot builder, and `plot_matplotlib_adapter.py` compatibility adapter. These do not change runtime plotting behavior yet; they prepare a stable renderer-facing surface before offscreen adapter smoke tests or PyQtGraph POC work.
