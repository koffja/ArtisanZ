...
| 5 | Local QML Islands | Started: workspace-status model + lazy dock entry | A limited Qt Quick panel or dashboard backed by Python state models | QML proves useful without breaking packaging, translations, or Windows portable build |
...
**Started:** 2026-07-01 with an isolated `WorkspaceStatusModel` `QObject` and a small QML status panel string compiled by an offscreen `QQmlComponent` test. Follow-up slices add a `QQuickWidget` factory that loads the inline QML through a data URL, injects the Python model, updates macOS/Linux/Windows PyInstaller hidden imports for QtQuick/QML modules, and exposes the island through a lazy View-menu dock entry. This validates the Python-to-QML state seam without moving the roast graph or settings dialogs to QML.
