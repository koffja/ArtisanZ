...
| 2 | Plot Renderer Boundary and PyQtGraph POC | Backend-aware live-frame dispatch and real PyQtGraph widget target factory wired for smoke coverage | Read-only plot snapshot model, Matplotlib adapter, PyQtGraph live adapter prototype | POC is benchmarkable before any live renderer swap |
